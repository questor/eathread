...
void EA::Thread::ThreadSleep(const ThreadTime& timeRelative)
{
	...
	else
	{
		...
		if (EASTL_UNLIKELY(tpThreadDynamicData == nullptr))
		{
		...
		if (EASTL_LIKELY((timeNanoSeconds < (SECONDS_TO_NANOSECONDS * 100)) && pThreadTimerQueue->mbEnabled))
		{
			const long kMinTimeForTimerEventNanoSeconds = 100000; // 100 microseconds represented in nanoseconds
			ts.tv_nsec = EASTL_UNLIKELY((ts.tv_nsec < kMinTimeForTimerEventNanoSeconds) && (ts.tv_sec != 0)) ?

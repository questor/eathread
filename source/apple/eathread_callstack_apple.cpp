...
#if EATHREAD_APPLE_GETMODULEINFO_ENABLED
// This fills a moduleInfoApple object with the information from all of the segments listed in the given mach_header's segments, starting at the given currentSegmentPos. It also puts the pModulePath info the moduleInfoApple object, which is then pushBack on the given array.
//
        ...
        if(pSegmentCommand != NULL && pSegmentCommand->cmd == kLCSegment) // If this really is a segment_command... (otherwise it is some other kind of command)
        {
            ...
            if(!pTypeFilter || strncmp(segnameBuffer, pTypeFilter, sizeof(segnameBuffer)))
            {
                ...
                if (arraySize < arrayCapacity)
                {
                    ...
                    strlcpy(info.mPath, pModulePath, EASTLArrayCount(info.mPath));
                    ...
                    strlcpy(info.mName, pDirSeparator, EASTLArrayCount(info.mName));
                    ...
                    strlcpy(info.mType,pSegmentCommand->segname,EASTLArrayCount(info.mType));
                    //**********************************************************************************

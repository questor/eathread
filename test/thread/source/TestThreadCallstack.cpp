...
{
	eastl::fixedVector<void*, 32> mAddressSet;
};
...
EASTL_NO_INLINE void TestCallstack01(CallstackTestInfo& callstackInfo, int& nErrorCount);
EASTL_NO_INLINE void TestCallstack02(CallstackTestInfo& callstackInfo, int& nErrorCount);
EASTL_NO_INLINE void TestCallstack03(CallstackTestInfo& callstackInfo, int& nErrorCount);
EASTL_NO_INLINE void TestCallstack04(CallstackTestInfo& callstackInfo, int& nErrorCount);
EASTL_NO_INLINE void TestCallstackWithContext02(CallstackTestThreadContext* context);
EASTL_NO_INLINE void TestCallstackWithContext01(CallstackTestThreadContext* context);
EASTL_NO_INLINE intptr_t TestCallstackContextThreadFunc(void* context);
...
static void VerifyCallstack(CallstackTestInfo& callstackInfo, EA::Thread::CallstackContext* context, int& nErrorCount)
{
	...
	size_t       addressCount = EA::Thread::GetCallstack(addressArray, EASTLArrayCount(addressArray), context);
	...
	{
		eastl::fixedString<char, 256> sExpectedCallstack;
		eastl::fixedString<char, 256> sReportedCallstack;

		for(eastl_size_t i = 0; i < callstackInfo.mAddressSet.size(); i++)
			sExpectedCallstack.appendSprintf("%p ", callstackInfo.mAddressSet[i]);

		for(size_t i = 0; i < addressCount; i++)
			sReportedCallstack.appendSprintf("%p ", addressArray[i]);
...
EASTL_NO_INLINE EACALLSTACK_TEST_FUNCTION_LINKAGE void TestCallstackWithContext02(CallstackTestThreadContext* context)
{
	int nErrorCount = 0;
	EA::UnitTest::NonInlinableFunction();
...
EASTL_NO_INLINE EACALLSTACK_TEST_FUNCTION_LINKAGE void TestCallstackWithContext01(CallstackTestThreadContext* context)
{
	void* pAddress;

	EA::UnitTest::NonInlinableFunction();
	EAGetInstructionPointer(pAddress);
	context->mCallstackTestInfo->mAddressSet.pushBack(pAddress);
...
EASTL_NO_INLINE intptr_t TestCallstackContextThreadFunc(void* context)
{
	...
	void* pAddress;
	EAGetInstructionPointer(pAddress);
	threadContext->mCallstackTestInfo->mAddressSet.pushBack(pAddress);
...
EASTL_NO_INLINE EACALLSTACK_TEST_FUNCTION_LINKAGE void TestCallstack04(CallstackTestInfo& callstackInfo, int& nErrorCount)
{
	void* pAddress;
	EAGetInstructionPointer(pAddress);
	callstackInfo.mAddressSet.pushBack(pAddress);
...
EASTL_NO_INLINE EACALLSTACK_TEST_FUNCTION_LINKAGE void TestCallstack03(CallstackTestInfo& callstackInfo, int& nErrorCount)
{
	void* pAddress;
	EAGetInstructionPointer(pAddress);
	callstackInfo.mAddressSet.pushBack(pAddress);
...
EASTL_NO_INLINE EACALLSTACK_TEST_FUNCTION_LINKAGE void TestCallstack02(CallstackTestInfo& callstackInfo, int& nErrorCount)
{
	void* pAddress;
	EAGetInstructionPointer(pAddress);
	callstackInfo.mAddressSet.pushBack(pAddress);
...
EASTL_NO_INLINE EACALLSTACK_TEST_FUNCTION_LINKAGE void TestCallstack01(CallstackTestInfo& callstackInfo, int& nErrorCount)
{
	void* pAddress;
	EAGetInstructionPointer(pAddress);
	callstackInfo.mAddressSet.pushBack(pAddress);
...
EASTL_NO_INLINE EACALLSTACK_TEST_FUNCTION_LINKAGE  int TestRemoteThreadContextVsCallstack()
{
	int nErrorCount(0);
	...
	struct CallstackTestThread : public EA::Thread::IRunnable
	{
		...
		intptr_t Run(void*)
		{
			EA::Thread::GetCallstack(mAddressCallstackArray, EASTLArrayCount(mAddressCallstackArray));
...

	{
		...
		{
			EA::Thread::GetCallstack(addressContextArray, EASTLArrayCount(addressContextArray), &callstackContext);
		}
	...
	{
		if (eastl::findIf(eastl::begin(addressContextArray), eastl::end(addressContextArray),

...
int TestThreadMisc()
{
	...
	nErrorCount += TestThreadGetThreadTimeMin();
		...
		size_t n = EA::Thread::GetModuleInfoApple(moduleInfoApple, EASTLArrayCount(moduleInfoApple), NULL, true);

		#if defined(EA_PLATFORM_OSX)
			EATEST_VERIFY(n > 0);
			for(size_t i = 0; i < eastl::min(n, EASTLArrayCount(moduleInfoApple)); i++)
